export default function HeaderBody() {
  return (
    <>
      <div id="home">
        <div className="background" id="back"></div>
        <h1 className="text-center">Hello. My Name is Yosef Melamed</h1>
        <div className="d-flex justify-content-center">
          <img id="profile" src="images/mypicture2.jpg" alt="" />
        </div>
        <h4 id="Home">
          I am a <span id="one">fullstack</span> developer, and{" "}
          <span id="two">I love to</span> work across the stack. I love to{" "}
          <span id="three">problem solve</span>, learn, and
          <span id="four"> teach</span>, and I am a{" "}
          <span id="five">strong team player.</span> I am{" "}
          <span id="six">passionate about developing</span> the web and
          harnessing its power to <span id="seven">impact our</span> daily lives
          and to <span id="eight">change the world </span>
          for the better.
        </h4>
      </div>
    </>
  );
}
